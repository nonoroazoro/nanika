...
impl ClipboardDatabase {
    ...
    pub fn upsert_with_retention(
        ...
        let removed = prune(&transaction, now, config)?;
    ...
    pub fn load(&self) -> Result<Vec<ClipboardEntry>, String> {
        let mut statement = self
            .connection
            .prepare(
                "SELECT entry_id, content_kind, title, text_payload, files_json,
        ...
        rows.into_iter()
            .map(
                |(entry_id, kind, title, text, files, image, size, captured)| {
                    Ok(ClipboardEntry {
                        entry_id,
                        title,
    ...
    pub fn clear(&self, entry_ids: &[String]) -> Result<ClipboardChange, String> {
        ...
        let mut removed = HashSet::new();
        {
            let mut statement = transaction
                .prepare("DELETE FROM clipboard_entries WHERE entry_id = ?1 RETURNING entry_id")
                .map_err(|error| error.to_string())?;
            for entry_id in entry_ids {
                for row in statement
                    ...
                    .map_err(|error| error.to_string())?
                {
                    removed.insert(row.map_err(|error| error.to_string())?);
                }
            }
        }
        let retained = image_paths(&transaction)?;
...
fn prune(
    ...
    if let Some(cutoff) = config.cutoff_millis(now) {
        let mut statement = connection
            .prepare("DELETE FROM clipboard_entries WHERE captured_at < ?1 RETURNING entry_id")
            .map_err(|error| error.to_string())?;
        for row in statement
            .query_map([integer(cutoff)], |row| row.get::<_, String>(0))
            .map_err(|error| error.to_string())?
        {
            removed.insert(row.map_err(|error| error.to_string())?);
        }
    ...
    if let Some(maximum) = config.max_entries {
        ...
        for row in statement
            .query_map([i64::from(maximum)], |row| row.get::<_, String>(0))
            .map_err(|error| error.to_string())?
        {
            removed.insert(row.map_err(|error| error.to_string())?);
        }
