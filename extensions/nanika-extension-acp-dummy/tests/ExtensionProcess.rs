...
use nanika_host::{ExtensionLimits, ExtensionRuntime, ExtensionRuntimeInvocation, SupervisorError};
...
fn host_forcibly_terminates_a_non_cooperative_cancelled_prompt() {
    let runtime = hanging_runtime(TEST_TIMEOUT);
    let (sender, receiver) = mpsc::sync_channel(1);
    let thread = std::thread::spawn(move || {
        let started_at = Instant::now();
        let mut runtime = runtime;
        let result = runtime.invoke_cancellable(hanging_invocation(), Arc::new(|_| {}), || true);
        let _ = sender.send((result, started_at.elapsed()));
    });

    let (result, elapsed) = receiver
        .recv_timeout(Duration::from_secs(3))
        .expect("cancelled prompt must return");
    assert!(matches!(
        result,
