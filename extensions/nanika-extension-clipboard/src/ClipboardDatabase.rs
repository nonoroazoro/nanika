...
use crate::{ClipboardEntry, EncodedClipboardContent, migrations::MIGRATIONS};
...
fn apply_migrations(connection: &mut Connection) -> Result<(), String> {
    ...
    for (index, version) in applied.iter().enumerate() {
        if *version != i64::try_from(index + 1).unwrap_or(i64::MAX) {
            return Err("clipboard database migration history is not contiguous".to_owned());
        }
    }
    if applied.len() > MIGRATIONS.len() {
        return Err("clipboard database was created by a newer extension".to_owned());
    }
