...
- Settings: a separate decorated Tauri window opened from the Root Search status bar. It is prepared hidden after runtime initialization and retains its WebView on close while discarding unsaved drafts.
...
- Settings: typed controls generated from each extension's validated `contributes.configuration` schema. Only extensions with nonempty settings appear in the sidebar. General edits the launcher shortcut, hide-on-blur, theme, and launch at login without requiring extensions. Extension management and ACP session configuration remain separate future work.
...
Collection previews use at most three equally sized transparent artwork slots without a card background, border, corner mask, or shadow. Artwork fits without cropping or changing its aspect ratio. Items paint in increasing stacking order from left to right; slight rotation and limited overlap preserve recognition of each item. Stacking is isolated to the preview. Clipboard publishes the selected preview group only after all of its at most three native icon requests have completed, including explicit failures. List row icons remain progressive. The frontend retains static file artwork until all images in that group have decoded or settled on their existing fallback, then reveals the group together without animation, timers, or extra image requests. A group transitions from placeholder to settled artwork once per resource identity in a WebView session. Remember only settled group identities, never image buffers. Revisiting a settled group starts directly in its final presentation state, before mount, load, or decode callbacks; changing selection or reopening the view must not replay its placeholder phase. A changed resource identity starts a new group. This presentation is shared by Windows and macOS and uses the existing cached images without extra image generation, animation, or filters.
...
Visual updates preserve the active surface's DOM whenever its presentation kind remains the same. While optimistic list selection waits for the extension, retain the last committed detail and mark its pane `aria-busy`; never clear and remount the preview merely because local selection leads the Channel snapshot. The replacement detail comes from the authoritative snapshot, while action targets continue to follow current selection intent. Collection artwork uses positional slots, and file-path metadata updates existing text slots; changing file identities must not destroy the entire preview. A route key is reserved for an actual navigation boundary. Native image completion is scoped to its current resource and callback, so a superseded decode cannot change the current selection. Selected image detail loads immediately rather than waiting for lazy-loading eligibility; offscreen application-list icons remain lazy. Image failure state belongs to the failed URL and must not hide a replacement resource. Selecting a different list item resets the detail scrollport to its top; metadata and thumbnail updates for the same item preserve scrolling.

Assess visual stability with both rendered behavior and layout-shift observations. Exercise rapid keyboard selection, query/refresh updates, cold and cached resources, image failures, filter changes, and short/long detail content. Preserve list selection by stable result identity during reorder. Measure fixed header/footer and preview geometry as well as CLS: user-driven content changes are expected, and CLS excludes recent-input shifts, so a zero score alone does not establish flicker-free rendering. Diagnostic observers and fixtures belong in development/test tooling, never persistent production polling.
...
Visible-entry preparation runs only when delivering a new authoritative search snapshot. Preparation completion, Channel acknowledgements, navigation changes, and diagnostic updates may wake delivery but must not reschedule preparation for the same snapshot. Delivery respects the existing acknowledgement backpressure before scheduling preparation; a completion wake alone must leave the host and extensions idle.
...
Settings uses the shared button styling used by launcher actions. Add folder, removal, Save, and Discard introduce no private button theme. Save and Discard appear only while edits are pending. General contains host preferences without duplicating the extension navigation. Applications and Scripts each expose only a directory list with a native Add folder picker; there is no generic entry editor or exclusion editor for these extensions. Directory controls derive from the extension's `format: "directory"` schema, not its ID.

Settings uses a compact sidebar, extension icon and title, grouped preference rows, and an explicit Save and Discard footer. Scalar preferences align labels and descriptions on the left and controls on the right. Switching sections preserves unsaved drafts. Save returns after validated configuration is persisted and submitted; extension application continues in the background. A window-scoped Channel reports completion or the concrete application failure without polling. Applications serves its existing catalog while directory discovery runs and publishes the replacement catalog after successful discovery. Configuration acknowledgements do not block the host worker's search responses, and configuration changes remain ordered per extension. Closing the native window discards unsaved drafts; an already accepted save continues to completion. Native close reloads saved values and the latest application state without waiting for discovery, preparing the retained window for reopening. Settings loads its own component graph without loading launcher views or creating a search session. Its native window becomes visible after the initial settings render, so its first visible frame contains the form or a concrete load error.

Manifest `name` and `icon` are required presentation metadata for built-in and external extensions alike. Names contain 1 to 128 UTF-8 bytes without control characters. Icons select the host-rendered catalog: `applications`, `calculator`, `clipboard`, `command`, `script`, or `extension`. Settings and static contributions share the renderer; Clipboard reuses the existing raster artwork. Arbitrary icon URLs, markup, and extension-supplied frontend code are not accepted. Metadata is available without activating an on-demand extension. Manifest and protocol versions remain at their initial version.


Settings has no name-only search box, empty-directory placeholder, or passive success status. Only failures need persistent feedback. Boolean controls share one accessible Switch with keyboard activation, visible focus, disabled state, semantic theme tokens, and reduced-motion support. Integer editing preserves raw text, validates before submission, and never clamps, rounds, or converts an empty or overflowing input to zero. Integer schemas may opt into `allowUnlimited`, represented by `null`; finite values retain their declared bounds. Clipboard count and age each expose this control independently, with finite defaults of 50 entries and 7 days.


Settings and launcher controls must not draw external focus rings or glow. Keyboard focus uses the control's existing border, fill, or selection treatment; pointer focus never adds a browser-style outline.
