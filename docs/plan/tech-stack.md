...
The host owns the native visual language through design tokens and reusable `egui` components. Current styling is implementation state until the native design system and its interaction states are approved from actual Windows and macOS captures and interaction tests.
