...
The host, application, and clipboard schemas use the current pre-release baseline directly. Host usage identity includes extension, entry, action, and query context, with an index for retention cleanup. Startup parses and validates extension metadata one row at a time, reports malformed rows, and continues with valid extensions.
