...
fn platform_visibility_filters_presentation_without_changing_defaults() {
    let contribution: ConfigurationContribution = serde_json::from_value(json!({
        ...
        "properties": {
            "common": {"type": "boolean", "title": "Common", "default": true},
            "windows": {"type": "boolean", "title": "Windows", "default": false, "platforms": ["windows"], "order": 20},
            "macos": {"type": "boolean", "title": "macOS", "default": false, "platforms": ["macos"], "order": 10}
        }
    })).unwrap();
...
fn rejects_unknown_duplicate_and_excessive_platforms() {
    for platforms in [
        ...
        let contribution: ConfigurationContribution = serde_json::from_value(json!({
            "title": "Platform settings",
            "properties": {"enabled": {"type": "boolean", "title": "Enabled", "default": true, "platforms": platforms}}
        })).unwrap();
